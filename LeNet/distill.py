...
def run(EPOCHS, lr, batch_size, alpha, T, resume=True):
    ...
    teacher_model.load_state_dict(torch.load('../ResNet/models/resnet152_best.pt'))
    student_model = Lenet5().to(device)
    if resume:
        student_model.load_state_dict(torch.load('./models/distilled_lenet5_best.pt'))
    ...
    torch.manual_seed(0)
    g = torch.Generator()
    train_dataloader_teacher = DataLoader(CIFAR10(root='../ResNet/cifar-10-python/', train=True, transform=transform_teacher),
                                          shuffle=True, generator=g, batch_size=batch_size, num_workers=4)
    torch.manual_seed(0)
    g = torch.Generator()
    train_dataloader_student = DataLoader(CIFAR10(root='../ResNet/cifar-10-python/', train=True, transform=transform_student),
                                          shuffle=True, generator=g, batch_size=batch_size, num_workers=4)
    test_dataloader = DataLoader(CIFAR10(root='../ResNet/cifar-10-python/', train=False, transform=transform_test),
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(student_model.state_dict(), './models/distilled_lenet5_last.pt')

        test_acc = evaluate(student_model, test_dataloader)
        if test_acc > max_test_acc:
            max_test_acc = test_acc
            torch.save(student_model.state_dict(), './models/distilled_lenet5_best.pt')
...
if __name__ == '__main__':
    run(EPOCHS=100, lr=0.001, batch_size=128, alpha=0.3, T=7, resume=False)
